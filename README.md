...
It currently only plays a few variants (Australian, Canfield, Klondike (draw 1 and draw 3), Freecell, Limited, Scorpion, Spider1, Spider2, Wasp). Use a command line flag (eg -v=Limited) to set the variant. There should be around 40 by the time I'm done.
...
It uses the Microsoft solitaire retro card set by default, but as these don't scale prettily it also has simplified scalable cards, set via a command line flag -c=retro or -c=scalable).
